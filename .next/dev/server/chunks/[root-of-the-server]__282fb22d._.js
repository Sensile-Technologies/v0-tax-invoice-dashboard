    ...
    `, [
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            ...
            data: {
                ...
                totals: {
                    total_quantity: bulkSales.reduce((sum, s)=>sum + Number(s.quantity || 0), 0),
                    total_amount: bulkSales.reduce((sum, s)=>sum + Number(s.total_amount || 0), 0),
                    total_entries: bulkSales.length,
                    kra_transmitted: bulkSales.filter((s)=>s.kra_status === 'success' || s.kra_status === 'transmitted').length,
                    kra_skipped: bulkSales.filter((s)=>s.kra_status === 'pending' || s.transmission_status === 'not_sent').length
                }
            }
        });
    } catch (error) {
