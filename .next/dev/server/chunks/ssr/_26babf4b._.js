...
function ItemCompositionPage() {
    ...
    const fetchItems = async ()=>{
        ...
        const { data, error } = await supabase.from("items").select("*").eq("branch_id", branch.id).eq("status", "active").neq("item_type", "composite").order("item_name");
        if (error) {
    ...
    const fetchCompositeItems = async ()=>{
        ...
        if (!selectedBranch) return;
        const branch = JSON.parse(selectedBranch);
        const { data: composites, error } = await supabase.from("items").select(`
        id,
        item_code,
        item_name,
        sale_price,
        tax_type
      `).eq("branch_id", branch.id).eq("item_type", "composite").order("item_name");
        if (error) {
        ...
        const compositesWithCompositions = await Promise.all((composites || []).map(async (composite)=>{
            const { data: comps } = await supabase.from("item_compositions").select(`
            parent_item_id,
            percentage,
...
//# sourceMappingURL=_26babf4b._.js.map