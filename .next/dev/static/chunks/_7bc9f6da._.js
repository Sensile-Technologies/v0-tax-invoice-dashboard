            ...
            async function loadData() {
                try {
                    ...
                    if (hasUnreconciled && shiftSales.length > 0) {
                        ...
                        outgoing = allStaff.filter({
                            "EndShiftPage.useEffect.loadData": (s)=>salesStaffIds.has(s.id)
                        }["EndShiftPage.useEffect.loadData"]).map({
                            "EndShiftPage.useEffect.loadData": (s)=>({
                                    id: s.id,
                                    name: s.full_name || s.username || 'Unknown'
                                })
                        }["EndShiftPage.useEffect.loadData"]);
                    } else if (incomingAttendantIds.length > 0) {
                        // Not reconcile mode - use incoming attendants from previous shift
                        outgoing = allStaff.filter({
                            "EndShiftPage.useEffect.loadData": (s)=>incomingAttendantIds.includes(s.id)
                        }["EndShiftPage.useEffect.loadData"]).map({
                            "EndShiftPage.useEffect.loadData": (s)=>({
                                    id: s.id,
                                    name: s.full_name || s.username || 'Unknown'
