...
- **Bulk Sales**: Fuel sold but not individually invoiced during a shift. Formula: `Bulk Sales = (Closing Reading - Opening Reading) - Invoiced Quantity`. During shift closure, the system calculates meter differences and generates KRA-compliant invoices automatically. Bulk sales are stored in the `sales` table with `source_system = 'meter_diff_bulk'` and `is_automated = true`. Amounts are split into denominations (KES 100-2,500) for KRA compliance. The HQ dashboard shows bulk sales statistics per branch (count, volume, amount) in the Performance Analytics section.
