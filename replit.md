...
- `invoice_number`: Internal invoice number (e.g., `CIV-000252`) - generated by the system
- `kra_cu_inv`: KRA CU Invoice Number (e.g., `KRACU0300003796/394`) - returned by KRA after successful transmission

The KRA receipt number (`rcptNo`) is the second part of `kra_cu_inv` after the `/`. When displaying receipt numbers on printed receipts or the mobile app, always use `kra_cu_inv.split('/')[1]` to show the KRA-verified receipt number that matches the QR code verification.