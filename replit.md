...
- **Centralized Item Management**: Items are managed at the vendor headquarters level to prevent duplicate item codes across branches. Items created at HQ (branch_id=NULL) appear in the vendor-wide catalog. HQ users can assign items to branches with custom pricing via the "Assign to Branch" button in the HQ Items catalog (/headquarters/items). Branches view assigned items via `/api/branch-items` endpoint which returns both catalog-assigned items and legacy branch-specific items. Branch items page distinguishes "Catalog" (assigned from HQ) vs "Legacy" (branch-created) items with different action handlers. Catalog items use `/api/branch-items` for enable/disable/remove operations; legacy items use `/api/items`. KRA code mappings: Item Types use cd_cls '24' (1=Raw Material, 2=Finished Product, 3=Service), Origins use cd_cls '12'.
...
- **Fuel Type Derived from Items**: The `fuel_type` column in nozzles, tanks, and sales tables is now derived from the linked item via `item_id`. All queries use `COALESCE(i.item_name, n.fuel_type)` pattern to get fuel type from the items table first, falling back to the legacy fuel_type column for backward compatibility. This ensures a single source of truth for fuel names.
