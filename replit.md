...
## KRA Receipt Number Mapping
The sales table contains two invoice number fields:
- `invoice_number`: Internal invoice number (e.g., `CIV-000253`) - generated by the system
- `kra_cu_inv`: KRA CU Invoice Number (e.g., `KRACU0300003796/253`) - stores SCU ID + our internal invoice number

**IMPORTANT**: The `kra_cu_inv` field uses our **internal invoice number** (from `invcNo` sent to KRA), NOT KRA's `rcptNo` from the API response. This is because:
1. KRA's QR code verification portal displays the invoice number we sent (our `invcNo`), not their internal `rcptNo`
2. The `rcptNo` returned by KRA is their cumulative receipt counter which differs from what's shown when scanning the QR code
3. Using our `invcNo` ensures the receipt number on printed invoices matches what customers see when verifying via QR code

The receipt number displayed on printed receipts should be derived from `kra_cu_inv.split('/')[1]` to match the QR code verification.