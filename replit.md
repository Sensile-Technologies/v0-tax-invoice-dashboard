...
- **Branch Controller ID Mapping**: Branches can be assigned a `controller_id` (PTS ID) to automatically match incoming pump callbacks. Configurable via HQ create branch dialog or Admin onboarding dialog. The pump-callback endpoint uses this to route transactions to the correct branch.
