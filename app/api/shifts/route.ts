...
async function processKraTransmissionsInBackground(salesForKra: SaleForKra[], shiftId?: string, branchId?: string): Promise<void> {
  ...
  if (branchId) {
    try {
      ...
      const message = kraFailCount === 0
        ? `All ${kraSuccessCount} bulk sale invoices (KES ${totalAmount.toLocaleString()}) transmitted to KRA successfully.`
        : `KRA transmission: ${kraSuccessCount} success, ${kraFailCount} failed out of ${salesForKra.length} bulk invoices.`
      ...
      await pool.query(
        `INSERT INTO notifications (user_id, type, title, message, reference_id)
         VALUES (NULL, $1, $2, $3, $4)`,
        [notificationType, title, message, shiftId]
      )
    } catch (notifErr) {
...
async function generateBulkSalesFromMeterDiff(
  ...
  const branchConfigResult = await client.query(
    `SELECT invoice_number FROM branches WHERE id = $1`,
    [branchId]
  )
  ...
  for (const reading of nozzleReadings) {
    ...
    for (const invoiceAmount of amountDenominations) {
      ...
      const saleResult = await client.query(
        ...
        [branchId, shiftId, staffId, reading.nozzle_id, nozzleItemId, invoiceNumber, receiptNumber,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const prevTankReadingsResult = await pool.query(
      `SELECT DISTINCT ON (tank_id) tank_id, closing_reading
       FROM shift_readings
    ...
    const prevTankClosings: Record<string, number> = {}
    ...
    for (const tank of tanksResult.rows) {
      ...
      await pool.query(
        `INSERT INTO shift_readings (shift_id, branch_id, reading_type, tank_id, opening_reading)
         VALUES ($1, $2, 'tank', $3, $4)`,
        [shiftId, branch_id, tank.id, openingReading]
      )
    }
    ...
    for (const nozzle of nozzlesResult.rows) {
      await pool.query(
        `INSERT INTO shift_readings (shift_id, branch_id, reading_type, nozzle_id, opening_reading)
         VALUES ($1, $2, 'nozzle', $3, $4)`,
        [shiftId, branch_id, nozzle.id, parseFloat(nozzle.initial_meter_reading) || 0]
      )
    }
...
export async function PATCH(request: NextRequest) {
  ...
  try {
        ...
        if (reading.nozzle_id && !isNaN(reading.closing_reading)) {
          ...
          const updateResult = await client.query(
            `UPDATE shift_readings 
             ...
             RETURNING id`,
            [reading.closing_reading, outgoingAttendantId, incomingAttendantId, rtt, selfFueling, prepaidSale, id, reading.nozzle_id]
          )
          
          if (updateResult.rows.length === 0) {
            await client.query(
              `INSERT INTO shift_readings (shift_id, branch_id, reading_type, nozzle_id, opening_reading, closing_reading, outgoing_attendant_id, incoming_attendant_id, rtt, self_fueling, prepaid_sale)
               VALUES ($1, $2, 'nozzle', $3, $4, $5, $6, $7, $8, $9, $10)`,
              [id, branchId, reading.nozzle_id, openingReading, reading.closing_reading, outgoingAttendantId, incomingAttendantId, rtt, selfFueling, prepaidSale]
            )
          }
        ...
        if (stock.tank_id && !isNaN(closingStock)) {
          ...
          const updateResult = await client.query(
            `UPDATE shift_readings 
             ...
             RETURNING id`,
            [closingStock, stockReceived, id, stock.tank_id]
          )
    ...
    if (banking && banking.length > 0) {
      ...
      const branchVendorResultForBanking = await client.query(
        'SELECT vendor_id FROM branches WHERE id = $1',
        [branchId]
      )
      ...
      for (const entry of banking) {
        if (entry.banking_account_id && entry.amount > 0) {
          ...
          const accountCheck = await client.query(
            ...
            [entry.banking_account_id, vendorIdForBanking]
          )
          if (accountCheck.rows.length === 0) {
          ...
          await client.query(
            `INSERT INTO shift_banking (shift_id, banking_account_id, amount, notes)
             VALUES ($1, $2, $3, $4)`,
            [id, entry.banking_account_id, entry.amount, entry.notes || null]
          )
        }
      }
    }
