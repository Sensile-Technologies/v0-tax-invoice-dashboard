...
export async function POST(request: Request) {
  const logger = createApiLogger("/api/kra/customers/save", "POST")

  try {
    const supabase = await createClient()
    const body = await request.json()

    const { tin, bhfId, custTin, custNm, ...customerData } = body
    ...
    console.error("[v0] Save customer error:", error)
